...
ssh -i %KEY_PATH% %SERVER_USER%@%SERVER_IP% "chmod +x %TARGET_DIR%/watchdog-server && cd %TARGET_DIR% && nohup ./watchdog-server -config config.json > server.log 2>&1 < /dev/null &"
