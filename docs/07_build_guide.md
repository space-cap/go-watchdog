...
# 3. 수집 서버 빌드 (bin/server.exe 생성)
go build -o bin/server.exe ./server

# 4. 자원 수집 에이전트 빌드 (bin/agent.exe 생성)
...
# 2. 수집 서버 빌드 (실행 파일: bin/server)
go build -o bin/server ./server

# 3. 자원 수집 에이전트 빌드 (실행 파일: bin/agent)
go build -o bin/agent ./agent

# 4. 실행 권한 부여
chmod +x bin/server bin/agent
```
...
| :--- | :--- | :--- |
| **Windows** | `bin/server.exe` <br> `bin/agent.exe` | 대시보드 구동용 서버 <br> 자원 감시 대상 Windows PC/서버 |
| **Ubuntu** | `bin/server` <br> `bin/agent` | 대시보드 구동용 Linux 서버 <br> 자원 감시 대상 Linux PC/서버 |
